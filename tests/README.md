...
| `global-name` | Every capitalised `Name.x` or `Name:x` reads from something that exists: a class `LuaManager$Exposer` refers to, a global the game or the mod assigns in Lua, or a local, parameter or loop variable of the same file. `InventoryItemFactory` is a Java class build 42 does not expose, so Unwire Car Battery indexed nil in its `OnCreate` after the engine had already destroyed the inputs. `method-name` could not see it, because `CreateItem` is still a method, only on a class Lua cannot reach. |
| `stash-building` | Each stash house's `buildingX`, `buildingY` lands in a ground floor room, read from the game's `.lotheader` files. Otherwise the engine finds no building to prepare. |
| `stash-crate` | Each cache crate's `contX`, `contY`, `contZ` lands in a room. The engine builds the crate there with no fallback and only stocks containers in the stash building's rooms, so a crate outside one spawns empty or not at all. Muldraugh and Westpoint shipped that way. |
